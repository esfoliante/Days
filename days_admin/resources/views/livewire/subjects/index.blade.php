<div>
    ...
    <div class="card card-accent-primary">
        <div class="card-header d-flex align-items-center justify-content-between">
            {{ __('Dashboard') }}
            <button type="button" class="btn btn-primary" wire:click="create">Nova disciplina</button>
        ...
        <div class="card-body">
            <table class="table">
                ...
                @forelse($subjects as $subjectItem)
                    <tr>
                        <th scope="row">{{ $subjectItem->id }}</th>
                        <td>{{ $subjectItem->name ?? '' }}</td>
						

                        <td>{{ $subjectItem->created_at->diffForHumans()  }}</td>

                        <td>
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <button type="button" wire:click="$emit('update', {{ $subjectItem }})" class="btn btn-warning">
                                    Editar
                                </button>
                                <button type="button" class="btn btn-danger"
                                        wire:click="$emit('openDestroy', {{ $subjectItem }})">Remover
                                </button>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"
                        id="exampleModalLabel">{{ isset($subject) ? __('Editar Subjects') : __('Novo Subjects') }}</h5>
            ...
            <div class="input-group mb-3">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit"
                            class="btn btn-primary">{{ isset($subject) ? __('Atualizar') : __('Criar') }}</button>
                </div>
                </form>
    ...
    <div class="modal fade" id="destroyModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('Remover Subjects') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
